...
if(n!=2 | n!=7):
    ...
    print('optonal attributes for emailing a search status. If omitted no emails generated ')
    exit()
...
numofnewlistings=0
activelistingsproductidarray=[]

for page in range(0,int(numberofboats),10):
    html_page = urllib.request.urlopen(base_url+"&No="+str(page))
    soup=BeautifulSoup((html_page.read()),"html.parser")
    all=soup.find_all("div",{"class":["listing row","listing premier row"]})

    counter=0
    for item in all:
        d={}
        #productid is the number of the listing for the boat from the search results
        for link in item.findAll('a', attrs={'href': re.compile("^https://au.yachtworld")}):
            result=link.get('data-reporting-click-product-id')
            href=link.get('href')
            if result not in activelistingsproductidarray:
                    productid=(link.get('data-reporting-click-product-id'))
                    activelistingsproductidarray.append(link.get('data-reporting-click-product-id'))
                    listingurl=(link.get('href'))
        ...
        if numofpriorlistings != 0:
            for boatnumber in range(0,numofpriorlistings):
                ...
                #productid is the number of the listing for the boat from the search results
                if productid == str(boat["Listing Number"]):
...
numofactivelistings = len(activelistingsproductidarray)
...
if(numofpriorlistings) == 0:
    ...
    for alllistings in range(0,numofpriorlistings,1):
        ...
        for activelistings in range(0,numofactivelistings,1):
            ...
            if (str((allboats["Listing Number"]))==str((activelistingsproductidarray[activelistings]))):
                #Boat still active
                boatstillactive=1
